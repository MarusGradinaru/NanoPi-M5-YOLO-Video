...
static inline int8_t nc1hwc2_get(const int8_t *tensor, const rknn_tensor_attr *attr, int c, int h, int w) {
    const int H  = attr->dims[2];
    const int W  = attr->dims[3];
    const int C2 = attr->dims[4];
    const int c1 = c / C2;
    const int c2 = c % C2;
    size_t index = (((size_t)c1 * H + h) * W + w) * C2 + c2;
    return tensor[index];
}
...
static int process_i8_nc1hwc2(
    const int8_t *box_tensor, const rknn_tensor_attr *box_attr,
    const int8_t *score_tensor, const rknn_tensor_attr *score_attr,
    const int8_t *score_sum_tensor, const rknn_tensor_attr *score_sum_attr,
    int grid_h, int grid_w, int stride, int dfl_len,
    ...
    float threshold)
{
    ...
    if (score_sum_tensor != nullptr)
      score_sum_thres_i8 = qnt_f32_to_affine(threshold, score_sum_attr->zp, score_sum_attr->scale);

    for (int i = 0; i < grid_h; i++) {
        for (int j = 0; j < grid_w; j++) {
            int max_class_id = -1;

            // Fast rejection using score_sum
            if (score_sum_tensor != nullptr) {
                int8_t sum = nc1hwc2_get(score_sum_tensor, score_sum_attr, 0, i, j);
                if (sum < score_sum_thres_i8) continue;
            }
            ...
            for (int c = 0; c < OBJ_CLASS_NUM; c++) {
                int8_t score = nc1hwc2_get(score_tensor, score_attr, c, i, j);
                if (score > score_thres_i8 && score > max_score) { max_score = score; max_class_id = c; }
            }

            if (max_score > score_thres_i8) {
                float box[4]; float before_dfl[dfl_len * 4];

                for (int k = 0; k < dfl_len * 4; k++) {
                    int8_t value = nc1hwc2_get(box_tensor, box_attr, k, i, j);
                    before_dfl[k] = deqnt_affine_to_f32(value, box_attr->zp, box_attr->scale);
                }
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold,
  float nms_threshold, object_detect_result_list *od_results, bool native_nc1hwc2)
{
    rknn_output *_outputs = (rknn_output *)outputs;
    rknn_tensor_mem **_native_outputs = (rknn_tensor_mem **)outputs;
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (native_nc1hwc2) {
          if (!app_ctx->is_quant) { printf("Native NC1HWC2 postprocess currently supports INT8 only\n"); return -1; }

          int score_sum_idx = (output_per_branch == 3) ? i * output_per_branch + 2 : -1;
          const int8_t *score_sum_native = nullptr;
          const rknn_tensor_attr *score_sum_attr = nullptr;

          if (score_sum_idx >= 0) {
            score_sum_native = (const int8_t *) _native_outputs[score_sum_idx]->virt_addr;
            score_sum_attr = &app_ctx->output_native_attrs[score_sum_idx];
          }

          validCount += process_i8_nc1hwc2(
            (const int8_t *)_native_outputs[box_idx]->virt_addr, &app_ctx->output_native_attrs[box_idx],
            (const int8_t *)_native_outputs[score_idx]->virt_addr, &app_ctx->output_native_attrs[score_idx],
            score_sum_native, score_sum_attr, grid_h, grid_w, stride, dfl_len, filterBoxes, objProbs, classId, conf_threshold);
        } else {
          if (app_ctx->is_quant) {
