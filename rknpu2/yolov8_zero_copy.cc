...
int init_yolov8_model_zc(const char *model_path, rknn_app_context_t *app_ctx) {
    int ret;
    int model_len = 0;
    char *model;
    rknn_context ctx = 0;
...
int release_yolov8_model_zc(rknn_app_context_t *app_ctx) {
    int ret;
...
int inference_yolov8_model_zc(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results, float box_conf_threshold) {
    int ret;
    ...
    if (!app_ctx->is_quant) {
      printf("Currently native zero-copy postprocess supports INT8 only!\n");
      return -1;
    }

    // Post-process directly from native NC1HWC2 RKNN buffers
    ret = post_process(app_ctx, (void *)app_ctx->output_mems, &letter_box,
      box_conf_threshold, nms_threshold, od_results, true);
    if (ret < 0) { printf("post_process native NC1HWC2 fail! ret=%d\n", ret); return ret; }
